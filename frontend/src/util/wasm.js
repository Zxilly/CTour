export function runWithModule(module) {
  const asmLibraryArg = {
    ...
    if (id) {
      ...
      if (runDependencyWatcher === null && typeof setInterval != "undefined") {
        ...
        runDependencyWatcher = setInterval(function () {
          ...
          let shown = false;
          for (let dep in runDependencyTracking) {
            if (!shown) {
              shown = true;
      ...
      get_char: function (tty) {
        if (!tty.input.length) {
          ...
          if (
            ...
            // Browser.
            result = Module["readline"](); // returns null on cancel
            if (result !== null) {
              result += "\n";
            }
          }
          ...
          tty.input = intArrayFromString(result, true, 0);
        }
    ...
    createDataFile: (parent, name, data, canRead, canWrite, canOwn) => {
      ...
      if (data) {
        if (typeof data == "string") {
          ...
          for (; i < len; ++i) arr[i] = data.charCodeAt(i);
          data = arr;
        }
  ...
  const FSNode = /** @constructor */ function (parent, name, mode, rdev) {
    if (!parent) {
  ...
  const readMode = 292 /*292*/ | 73; /*73*/
  const writeMode = 146; /*146*/
  ...
  /** @type {function(...*):?} */
  const ___wasm_call_ctors = (Module["___wasm_call_ctors"] =
  ...
  /** @type {function(...*):?} */
  const ___errno_location = (Module["___errno_location"] =
  ...
  /** @type {function(...*):?} */
  const _malloc = (Module["_malloc"] = createExportWrapper("malloc"));

  /** @type {function(...*):?} */
  const _free = (Module["_free"] = createExportWrapper("free"));
  ...
  /** @type {function(...*):?} */
  const stackAlloc = (Module["stackAlloc"] = createExportWrapper("stackAlloc"));
  ...
  Module["FS_createDevice"] = FS.createDevice;
  Module["FS_unlink"] = FS.unlink;
  const unexportedRuntimeSymbols = [
    "run",
    "UTF8ArrayToString",
    "UTF8ToString",
    "stringToUTF8Array",
